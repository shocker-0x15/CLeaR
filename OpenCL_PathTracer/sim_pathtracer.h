...
//            if (gid0 == 340 && gid1 == 512) {
            while (rayIntersection(&scene, &ray.org, &ray.dir, &isect)) {
                ...
                if (hasNonSpecular((BSDFHead*)BSDF)) {
                    ...
                    if (lIsect.faceID == isect.faceID) {
                        *pix += alpha * Le(EDF, &lightDir) * fs(BSDF, &vout, &lightDirRev) * absCosNsBSDF((BSDFHead*)BSDF, &lightDir) * absCosNsEDF((EDFHead*)EDF, &lightDirRev) / dist2;
                    }
                }
