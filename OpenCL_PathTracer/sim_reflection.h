...
namespace sim {
    ...
    void BSDFAlloc(const Scene* scene, uint offset, const Intersection* isect, uchar* BSDF) {
        ...
        bool hasBumpMap = *(bool*)AlignPtrAddG(&matsData_p, sizeof(bool));;
        uint bumpMapIdx = *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint));
        if (hasBumpMap) {
            ...
            *(BSDFp++) = BxDFID;
            switch (BxDFID) {
                ...
                case BxDFID_SpecularTransmission: {
                    ...
                    //Reflectance
                    color T = evaluateColorTexture(scene->texturesData + *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint)), isect->uv);
                ...
                case BxDFID_AshikhminS: {
                    ...
                    color Rs = evaluateColorTexture(scene->texturesData + *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint)), isect->uv);
                    memcpy(AlignPtrAdd(&BSDFp, sizeof(color)), &Rs, sizeof(color));
                    
                    //Exponent
                    float nu = evaluateFloatTexture(scene->texturesData + *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint)), isect->uv);
                    memcpy(AlignPtrAdd(&BSDFp, sizeof(float)), &nu, sizeof(float));
                    float nv = evaluateFloatTexture(scene->texturesData + *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint)), isect->uv);
                ...
                case BxDFID_AshikhminD: {
                    ...
                    //Reflectance
                    color Rd = evaluateColorTexture(scene->texturesData + *(uint*)AlignPtrAddG(&matsData_p, sizeof(uint)), isect->uv);
