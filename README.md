...
For now, it looks like the same as logic used in CPU implementation, so it is not efficient implementation for current GPU architectures as of 2014 due to a massive usage of registers and control-flow divergence by conditional branches [1].
It uses BVH for a spatial partitioning structure. Parallel construction of Linear BVH [2, 3] and ~~Treelet Restructuring BVH~~ (under development) [4] on GPU are implemented in addition to simple midpoint splitting on CPU.
